...
def vilnius(request):
    ...
    if request.method == "POST":
        ...
        if my_form.is_valid():
            ...
            items = groups.objects.filter(group_category='vilnius')
            for item in items.values('group_name', 'group_link', 'group_category'):
                ...
                link = 'https://facebook.com/groups/'+str(group_link)
                time.sleep(3)
                pyperclip.copy(link)
    ...
    return render(request, 'main/vilnius.html', context)  # How to render this page
